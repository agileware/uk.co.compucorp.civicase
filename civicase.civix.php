...
spl_autoload_register('_civicase_civix_class_loader', TRUE, TRUE);

function _civicase_civix_class_loader($class) {
  if ($class === 'CRM_Civicase_DAO_Base') {
    if (version_compare(CRM_Utils_System::version(), '5.74.beta', '>=')) {
      class_alias('CRM_Core_DAO_Base', 'CRM_Civicase_DAO_Base');
 ...
 * @return bool
 */
function _civicase_civix_insert_navigation_menu(&$menu, $path, $item) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _civicase_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
 ...
 */
function _civicase_civix_navigationMenu(&$nodes) {
  if (!is_callable(['CRM_Core_BAO_Navigation', 'fixNavigationMenu'])) {
    _civicase_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _civicase_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
...
function _civicase_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
